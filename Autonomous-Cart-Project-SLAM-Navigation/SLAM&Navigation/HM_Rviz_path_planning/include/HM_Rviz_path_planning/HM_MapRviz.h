...
public:
	explicit HMMapRviz(const std::string _map_filename)
	{
		...
		goal_pose_subscriber = nh.subscribe("/move_base_simple/goalfake", 5, &HMMapRviz::SetGoalpose, this);

		point_pose_subscriber = nh.subscribe("/clicked_point", 1, &HMMapRviz::SetpointArraypose, this);

		way_pose_subscriber = nh.subscribe("/move_base_simple/goals", 10, &HMMapRviz::SetpointArraypose_ver2, this);

		//SetpointArrayposes_ver3
		ways_pose_subscriber = nh.subscribe("/sampling/goals", 10, &HMMapRviz::SetpointArrayposes_ver3, this);
		...
		marker_publisher = nh.advertise<visualization_msgs::Marker>("visualization_marker", 1000);
